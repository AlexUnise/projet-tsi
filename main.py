...
def main():
    ...
    m = Mesh.load_obj('barrett.obj') #importation de la maille du stegosaure
    ...
    tr = Transformation3D()
    tr.translation.y = -np.amin(m.vertices, axis=0)[1]+0.5 #les pieds de l'objet à 0, le 1 represente le y
    tr.translation.z = -5
    ...
    o = Text('R pour spawn des ennemis', np.array([-0.8, 0.3], np.float32), np.array([0.8, 0.8], np.float32), vao, 2, programGUI_id, texture)
    viewer.add_object(o)
